...
<div class="cart-item-details-grid">
    ...
    <div class="product-quantity">
        <span>
        Quantity: <span class="quantity-label js-quantity-label-${matchingProduct.id}">${cartItem.quality}</span>
        </span>
...
document.querySelectorAll('.save-quantity-link')
  .forEach((link) => {
    link.addEventListener('click', () => {
      ...
      if(newQuantity <= 0 ) { 
        alert("Quality must be atleast 1 or more") 
        return
    } ;
      if(newQuantity >= 1000 ) { 
        alert("Quality must be atleast be below 1000") 
        return
    } ;
